import { 
  ...
  savedProperties, otps, bookings, projects, referrals, type User, type InsertUser, 
  ...
  type Booking, type InsertBooking, userRoles, type Project, type InsertProject,
...
export interface IStorage {
  ...
  getNewLaunchProjects(limit?: number): Promise<Project[]>;
  getCommercialProjects(limit?: number): Promise<Project[]>;
  getUpcomingProjects(limit?: number): Promise<Project[]>;
  getLuxuryProjects(limit?: number): Promise<Project[]>;
  getAffordableProjects(limit?: number): Promise<Project[]>;
  ...
  updateReferralStatus(id: number, status: string, notes?: string): Promise<Referral | undefined>;
...
export class MemStorage implements IStorage {
  ...
  async getProjectsByCategory(category: string, limit: number = 10): Promise<Project[]> {
    const projects = Array.from(this.projects.values())
      .filter(project => project.category === category && project.approvalStatus === "approved")
      .sort((a, b) => {
        if (a.createdAt && b.createdAt) {
          return b.createdAt.getTime() - a.createdAt.getTime();
        }
  ...
  async getFeaturedProjects(limit: number = 6): Promise<Project[]> {
    const featured = Array.from(this.projects.values())
      .filter(project => project.featured && project.approvalStatus === "approved")
      .sort((a, b) => {
        if (a.createdAt && b.createdAt) {
          return b.createdAt.getTime() - a.createdAt.getTime();
        }
  ...
  async getNewLaunchProjects(limit: number = 6): Promise<Project[]> {
    const newLaunches = Array.from(this.projects.values())
      .filter(project => project.category === 'new_launch' && project.approvalStatus === "approved")
      .sort((a, b) => {
        if (a.createdAt && b.createdAt) {
          return b.createdAt.getTime() - a.createdAt.getTime();
        }
  ...
  async getCommercialProjects(limit: number = 6): Promise<Project[]> {
    const commercial = Array.from(this.projects.values())
      .filter(project => project.category === 'commercial' && project.approvalStatus === "approved")
      .sort((a, b) => {
        if (a.createdAt && b.createdAt) {
          return b.createdAt.getTime() - a.createdAt.getTime();
        }
  ...
  async getUpcomingProjects(limit: number = 6): Promise<Project[]> {
    const now = new Date();
    const upcoming = Array.from(this.projects.values())
      .filter(project => 
        project.category === 'upcoming' && 
        project.approvalStatus === "approved" && 
        project.possessionDate && 
        new Date(project.possessionDate) > now
      )
      .sort((a, b) => {
        if (a.possessionDate && b.possessionDate) {
          return new Date(a.possessionDate).getTime() - new Date(b.possessionDate).getTime(); // Sort by closest possession date
        }
        return 0;
      });
  ...
  async getLuxuryProjects(limit: number = 6): Promise<Project[]> {
    const luxury = Array.from(this.projects.values())
      .filter(project => project.category === 'luxury' && project.approvalStatus === "approved")
      .sort((a, b) => {
        // Sort by price (highest first), parse price range
        const priceA = parseFloat(a.price || '0');
        const priceB = parseFloat(b.price || '0');
        return priceB - priceA;
      });
      
    return luxury.slice(0, limit);
  }
  
  async getAffordableProjects(limit: number = 6): Promise<Project[]> {
    const affordable = Array.from(this.projects.values())
      .filter(project => project.category === 'affordable' && project.approvalStatus === "approved")
      .sort((a, b) => {
        // Sort by price (lowest first), parse price range
        const priceA = parseFloat(a.price || '0');
        const priceB = parseFloat(b.price || '0');
  ...
  async searchProjects(query: {
    ...
    return Array.from(this.projects.values()).filter(project => {
      ...
      if (query.city && project.city.toLowerCase() !== query.city.toLowerCase()) {
        return false;
      }
      ...
      if (query.builder && project.builder.toLowerCase() !== query.builder.toLowerCase()) {
        return false;
      }
      
      if (query.bhkConfig) {
        const bhkConfig = query.bhkConfig.toLowerCase();
        if (!project.bhkConfig.toLowerCase().includes(bhkConfig)) {
          return false;
        }
      }
      ...
      if (query.priceRange) {
        ...
        if (!isNaN(min) && !isNaN(max)) {
          // Check if project price range overlaps with query range
          const projectPrice = parseFloat(project.price || '0');
          const priceRangeStr = project.priceRange || '';
          
          // If project has a price range string like "2000000-4000000"
          if (priceRangeStr.includes('-')) {
            const [projectMinStr, projectMaxStr] = priceRangeStr.split('-');
            const projectMin = parseFloat(projectMinStr);
            const projectMax = parseFloat(projectMaxStr);
            
            if (isNaN(projectMin) || isNaN(projectMax) || 
                (projectMax < min || projectMin > max)) {
              return false;
            }
          } 
          // Otherwise just check the price
          else if (projectPrice < min || projectPrice > max) {
            return false;
          }
        }
      }
