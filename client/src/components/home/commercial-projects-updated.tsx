...
import { CommercialProperty } from "@/types/property-types";
...
export default function CommercialProjects() {
  ...
  const { data: commercialProperties, isLoading } = useQuery<CommercialProperty[]>({
    queryKey: ["/api/projects/commercial"],
...
function CommercialPropertyCard({ property }: CommercialPropertyCardProps) {
  ...
  return (
    <Link href={`/project-detail/${property.id}`} onClick={() => window.scrollTo(0, 0)}>
      <div className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300">
        ...
        <div className="relative w-full h-32 sm:h-40 md:h-44">
          ...
          <img
            ...
            onLoad={() => {
              console.log(`Commercial property image loaded successfully`);
            }}
            onError={(e) => {
              console.log(`Error loading commercial property image`);
              setImageError(true);
              handleImageError(e, placeholderImage, true);
            }}
          />
        </div>
